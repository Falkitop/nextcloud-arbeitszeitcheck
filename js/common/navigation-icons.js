/**
 * Navigation icon handling for ArbeitszeitCheck
 *
 * Replaces all elements with a `data-lucide` attribute inside the app
 * navigation with local SVG icons. This avoids loading third-party
 * icon libraries at runtime while keeping the markup declarative.
 */
(() => {
	'use strict';

	const NAV_ICON_SELECTOR = '[data-lucide]';
	...
	const arbeitszeitcheckNavSvgIcons = {
		...
		'flask-conical': '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="lucide-icon"><path d="M10 2v7.527a2 2 0 0 1-.211.896L4.72 16.55a1 1 0 0 0 .895 1.45h12.79a1 1 0 0 0 .895-1.45l-5.069-6.127A2 2 0 0 1 14 9.527V2"/><path d="M8.5 2h7"/><path d="M7 16h10"/></svg>',
	...
	function applyNavigationIcons(root) {
		...
		const nodes = container.querySelectorAll(NAV_ICON_SELECTOR);
		...
		nodes.forEach((el) => {
			...
			const svg = arbeitszeitcheckNavSvgIcons[iconName];
			if (svg) {
				el.innerHTML = svg;
			}
		});
	...
	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', () => applyNavigationIcons(document));
	} else {
		applyNavigationIcons(document);
	}
})();

/**
 * Navigation icon rendering for ArbeitszeitCheck.
 *
 * Replaces <i data-lucide="..."> placeholders in the navigation
 * with locally defined SVG markup. This was previously implemented
 * as an inline <script> block in the navigation template and is now
 * moved into this bundled JavaScript file for CSP compliance.
 */
...
(function () {
	...
	document.addEventListener('DOMContentLoaded', () => {
		document.querySelectorAll('[data-lucide]').forEach((el) => {
			const iconName = el.getAttribute('data-lucide');
			if (iconName && Object.prototype.hasOwnProperty.call(arbeitszeitcheckNavSvgIcons, iconName)) {
