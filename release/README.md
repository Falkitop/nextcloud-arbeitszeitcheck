...
If `make release-signed` fails because host `occ` cannot run, use the Docker signing fallback in `APPSTORE-RELEASE.md`; it overwrites **`build/release/arbeitszeitcheck-X.Y.Z.tar.gz`** so checksums, App Store signature, and `gh release create` all stay aligned.
