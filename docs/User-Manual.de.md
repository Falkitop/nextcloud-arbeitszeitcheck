...
- **Anteiliger Urlaub bei unterjährigem Ein-/Austritt:** Unter **Mitarbeitende → Bearbeiten** hat jede Person ein optionales **Eintrittsdatum** und **Austrittsdatum**. Deckt das Beschäftigungsverhältnis nicht das ganze Kalenderjahr ab, wird der Jahresanspruch (aus der jeweils greifenden Ebene) auf den beschäftigten Teil des Jahres gekürzt. Sind beide Felder leer, bleibt der volle Anspruch erhalten (unverändertes Verhalten). Die Methode wird global unter **Admin-Benachrichtigungen → Anteiliger Urlaub** eingestellt: *Volle Monate (Zwölftelung, deutscher Standard)* gewährt 1/12 je angefangenem Beschäftigungsmonat und rundet einen Bruchteil von mindestens einem halben Tag auf einen vollen Tag auf (§ 5 BUrlG); *Exakte Tage* verwendet `Jahresanspruch × beschäftigte Tage ÷ Tage des Jahres`. Dies ist keine Rechtsberatung – prüfen Sie Ihren Tarifvertrag mit der Personalabteilung.
...
Auf der Seite **Meine Abwesenheiten** (o. Ä.) kann neben dem Jahresanspruch die Aktion **Wie wird das berechnet?** erscheinen. Der Dialog zeigt eine **redigierte** Erklärung (welche Ebene greift, ohne interne HR-Kennungen fremder Personen). Bei unterjährigem Ein- oder Austritt zeigt der Dialog zusätzlich den **gekürzten (anteiligen)** Wert für dieses Jahr mit einem kurzen Hinweis (z. B. „20 von 30 Tagen, beschäftigt 8 von 12 Monaten“), sodass die Zahl stets Ihrem nutzbaren Saldo entspricht.
...
- **Arbeitszeit erfassen** (manuell): Unter **Zeiteinträge** **Arbeitszeit erfassen** (oder einen bestehenden Eintrag bearbeiten). Das Formular ist in Abschnitte gegliedert: **Zusammenfassung** (Stunden und Compliance während der Eingabe), **Datum und Uhrzeit** (24-Stunden-Uhr; **Heute** setzt das Datum), **Pausen (optional)** und optional **Notiz**. Standardmäßig ist **Erforderliche Pausen automatisch ergänzen** aktiv: Die App kann Pausenzeiten einfügen, die den gesetzlichen Pausenregeln entsprechen (30 Minuten ab 6 Stunden Arbeit, 45 Minuten ab 9 Stunden). Schalten Sie dies nur aus, wenn Sie jede Pause selbst eintragen möchten. Automatisch ergänzte Pausen sind hervorgehoben und in der Zeile erklärt. Nachtschicht: Endet die Arbeit nach Mitternacht, ggf. den **nächsten Kalendertag** als Arbeitstag verwenden.
