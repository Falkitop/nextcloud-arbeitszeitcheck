...
- **Pro-rata vacation for partial years (mid-year join/leave):** Under **Employees → Edit**, each person has an optional **Employment start date (Eintrittsdatum)** and **Employment end date (Austrittsdatum)**. When the employment does not cover the whole calendar year, the annual entitlement (from any layer above) is reduced to the worked part of the year. Leave both empty to keep the full entitlement (unchanged behaviour). The reduction method is global, set on **Admin notifications → Pro-rata vacation**: *Full months (Zwölftelung, German default)* grants 1/12 per calendar month touched by the employment and rounds a half-day-or-more fraction up to a full day (BUrlG §5); *Exact days* uses `annual × worked days ÷ days in year`. This is not legal advice — confirm your collective agreement with HR.
...
On **My absences** (or equivalent), next to the annual entitlement figure, use **How is this calculated?** when shown. The dialog loads a **redacted** explanation (which layer applied, no internal HR-only identifiers) so you can see *that* the number comes from a configured rule without exposing other people’s data. If you joined or left during the year, the dialog also shows the **reduced (pro-rata)** figure for that year with a short note (for example, “20 of 30 days, employed 8 of 12 months”), so the number always matches your usable balance.
...
- **Record working time** (manual entry): On **Time entries**, use **Record working time** (or edit an existing entry). The form is split into clear sections: **Summary** (hours and compliance while you type), **Date and time** (24-hour clock; **Today** sets the date), **Breaks (optional)**, and an optional **Note**. By default, **Automatically add required breaks** is on: the app can insert pause times that meet German break rules (30 minutes from 6 hours worked, 45 minutes from 9 hours). Turn this off only if you want to enter every break yourself. Auto-added breaks are highlighted and explained on the row. Night shifts: if you finish after midnight, use the next calendar day as the work date when needed.
